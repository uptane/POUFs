...
The Director repository identifies a vehicle at the start of the update process using the vehicle version manifest sent by the Primary ECU. The Director checks the vehicle version manifest as described in the standard, but does not make any additional checks. Using the information in the vehicle version manifest, the director determines what images need to be installed.
...
* If images are encrypted per ECU, metadata about encrypted images may be included in the custom field of targets metadata as described in the [Standard](https://uptane.github.io/uptane-standard/uptane-standard.html#custom-metadata-about-images). Targets metadata from the Director will still include metadata about unencrypted images so that this metadata can be compared with the metadata from the Image repository.
...
* The Director repository is initialized with the private keys for all roles with online keys. These keys are stored on the Director so they can be used to automatically sign valid metadata.
