...
The Director repository identifies a vehicle at the start of the update process using a stored vehicle version manifest. The Director checks the vehicle version manifest as described in the standard, but does not make any additional checks. This POUF does not do dependency resolution as the reference implementation is not used in practice and so does not have conflicts.

This POUF does not support sending vehicle version manifests as [diffs](https://uptane.github.io/uptane-standard/uptane-standard.html#construct_manifest_primary) as the reference implementation is not concerned about potential bandwidth issues.
...
* The OEM will provide unencrypted images to ECUs, hence the Director repository’s targets metadata will contain hashes of unencrypted images. Metadata about encrypted images may be included in the custom field of targets metadata as described in the [Standard](https://uptane.github.io/uptane-standard/uptane-standard.html#custom-metadata-about-images).
...
* The custom field of targets metadata on the Director and Image repositories are not compared. This means that the must match functionality described in the [Standard](https://uptane.github.io/uptane-standard/uptane-standard.html#custom-metadata-about-images) is not supported. A client may choose to compare some fields of targets metadata, but these checks are not required by this POUF.
* The Image repository’s targets metadata may include metadata about delegations. [Delegations may be any depth and may also be cyclic](https://uptane.github.io/uptane-standard/uptane-standard.html#delegations_meta).
* Snapshot metadata in this POUF does not include the root filename or version.

# Usage
As the Uptane reference implementation is not used in practice, key management and image generation are not handled by this POUF. The POUF uses online keys for all roles to demonstrate Uptane functionality. This practice is not recommended for Uptane implementers.

## Server Setup
The Uptane Reference Implementation does the following setup before any updates can be installed:
* The Director repository is initialized with the private keys for all roles with online keys.
